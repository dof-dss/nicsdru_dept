  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    foreach ($departments->getInstalledContentPlugins() as $plugin) {
      if ($plugin->getEntityTypeId() === 'node') {
        $bundles[] = $plugin->getEntityBundle();
      }
    }
    ...
    $batch_builder
      ->setTitle($this->t('Creating node batches for @num bundles', ['@num' => $bundles,]))
      ->setFile(\Drupal::service('extension.list.module')->getPath('dept_etgrm') . '/etgrm.batch.inc')
      ->setFinishCallback('buildRelationshipBatchFinished')
      ->setErrorMessage(t('Batch has encountered an error'));

    foreach ($bundles as $bundle) {
      $batch_builder->addOperation('buildRelationshipBatch', [
        $bundle,
        $etgrm_service,
      ]);
    }
...
//    $etgrm = new EntityToGroupRelationshipManagerService();
//    $etgrm->create()->all();
//
//    foreach ($etgrm->batch as $batch) {
//
//    }
//
//

...
if [ -n "${GOOGLE_APPLICATION_CREDENTIALS_JSON}" ]; then
   if echo "$GOOGLE_APPLICATION_CREDENTIALS_JSON" | jq empty >/dev/null 2>&1; then
          ...
          echo "Environment variable 'GOOGLE_APPLICATION_CREDENTIALS_JSON' contains invalid JSON."
          exit 1
   fi
else

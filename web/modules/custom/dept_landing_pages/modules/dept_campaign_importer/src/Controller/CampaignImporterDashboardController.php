...
namespace Drupal\dept_campaign_importer\Controller;
...
/**
 * Returns responses for Dept Campaign Utilities routes.
 */
  ...
  public function build() {
    ...
    foreach ($d7_landing_pages as $landing_page) {
      ...
      $item = [
        ...
        'drupal7' => Link::fromTextAndUrl('View', Url::fromUri('https://www.northernireland.gov.uk/node/' . $landing_page->nid)),
      ...
      if ((count($d8nids) === 1)) {
        $item['update'] = Link::createFromRoute('Update', 'dept_campaign_importer.creator',
          ['nid' => $landing_page->nid],
          [
            'query' => [
      ...
      $item['create'] = Link::createFromRoute('Create', 'dept_campaign_importer.creator',
        ['nid' => $landing_page->nid],
        [

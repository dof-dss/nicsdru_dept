...
final class LandingPagesChooseBlockController implements ContainerInjectionInterface {
  ...
  public function inlineBlockList(SectionStorageInterface $section_storage, $delta, $region) {
    ...
    foreach ($build['links']['#links'] as &$link) {
      ...
      $link['attributes']['class'][] = 'dept-landing-pages--add-block';
    }

    // Additional styling for the back link.
    $build['back_button']['#attributes']['class'][] = 'dept-landing-page--button-back';

    // Add sidebar title.
    $build['#title'] = $this->t('Select a custom block type');

    $build['links']['#attributes']['class'][] = 'dept-landing-pages';

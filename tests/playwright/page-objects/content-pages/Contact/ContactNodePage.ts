...
export class ContactNodePage
{
    ...
    async contactNodeURLCheck()
    {
        ...
        try
        {
            await this.testSteps.LogInfo(`Verifying URL path is /contacts/${escapeRegex(this.testSetUpData.contentTitleforTest.contentTitle)}$`);
            await expect(this.page).toHaveURL(
                new RegExp(this.testSetUpData.urlForTest.url + `/contacts/${escapeRegex(this.testSetUpData.contentTitleforTest.contentTitle)}$`)
            );
        }
        catch
        {
            await this.testSteps.LogInfo(`URL is not /contacts/${escapeRegex(this.testSetUpData.contentTitleforTest.contentTitle)}$ but is /node/.+/latest`);
    ...
    async verifyContact({ topics }: VerifyOptions)
    {
        // verify title
    ...
    async verifyEditedContact({ topics }: VerifyOptions)
    {
        // verify title
